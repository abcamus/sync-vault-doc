...
A **TURN server** acts as a "relay station" or "translator" that enables direct communication between devices behind network "firewalls" or "NAT". When two devices cannot establish a direct "handshake" connection, all their data is relayed through this intermediate server. When using [[sync-modes/peer-to-peer-livesync | Real-Time Sync]], if devices fail to discover each other directly, a TURN server is required for data relay.
